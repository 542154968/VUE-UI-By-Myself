...
<template>
  <div
    ...
    @wheel="handleWheel"
  >
    <div class="wrap-contain" ref="wrap">
      <ul ref="ul" v-for="(item, i) in dataList" :key="i">
        <slot :data="item"></slot>
      </ul>
    </div>
  </div>
  ...
  methods: {
    ...
    handleWheel(e) {
      ...
      if (this.intervalScroll) {
        ...
        this.curScroll =
          e.deltaY < 0
            ? this.curScroll - this.mouseScrollSpeed
            : this.curScroll + this.mouseScrollSpeed;
        if (-this.curScroll >= 0) {
    ...
    setData() {
      ...
      this.dataList = new Array(
        curHeight > this.getNum(this.height) ? 2 : 1
      ).fill(this.datas);
      this.startScroll();
    },
    // 序列成数字
    ...
    handleIntervalUpMove() {
      ...
      this.$refs.wrapContain.style.transform = `translate3d(0, ${
        this.curScroll
      }px, 0)`;
    },
    ...
    handleIntervalMove() {
      cancelAnimationFrame(this.timeId);
      this.intervalAnimatTimeId = requestAnimationFrame(
        this.handleIntervalMove
      );
      this.curScroll = this.curScroll + this.speed;
      if (this.curScroll >= this.targetScroll) {
